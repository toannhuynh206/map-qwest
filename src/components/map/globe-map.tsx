...
// Small countries absent from the 110m TopoJSON — rendered as dots
...
export const GlobeMap = forwardRef<GlobeMapHandle, GlobeMapProps>(function GlobeMap(
  ...
  const onDragStart = useCallback((x: number, y: number) => {
    ...
    pendingSca.current = scaleRef.current;
  ...
  const onDragMove = useCallback((x: number, y: number) => {
    ...
    if (Math.sqrt(dx * dx + dy * dy) > TAP_THRESHOLD) wasDrag.current = true;
    pendingRot.current = [
  ...
  // ── Pointer events (touch-action:none → no OS scroll delay) ──────────────
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    ...
    if (pts.size === 2) {
      dragState.current = null;
      pinchRef.current  = { dist: getPointerDist(pts), scale: scaleRef.current };
  ...
  const onWheel = useCallback((e: React.WheelEvent) => {
    pendingSca.current = Math.max(SCALE_MIN, Math.min(SCALE_MAX, scaleRef.current * (1 - e.deltaY * 0.001)));
    scheduleFrame();
  }, [scheduleFrame]);
  ...
  const flyTo = useCallback((alpha3: string) => {
    ...
    const startSca = scaleRef.current;
    const targetSca = isMicro ? 1200 : startSca; // only zoom for micro-nations
    ...
    >
      ...
      >
        ...
        <Geographies geography={WORLD_TOPO}>
          {({ geographies }) => {
            // Track which alpha3 codes were rendered as polygons so we can
            // skip the dot fallback for those countries below.
            const renderedAsPolygon = new Set<string>();

            const elements = geographies.map((geo) => {
              const alpha3 = NUMERIC_TO_ALPHA3[String(geo.id)];
              if (!alpha3) return null;
              renderedAsPolygon.add(alpha3);
                ...
                <Geography
                  ...
                  style={{
                    default: {
                      ...
                      opacity: dimmedCountries.has(alpha3) && !fb ? 0.35 : 1,
                    },
            ...
            // Dot fallback — only for countries absent from the 50m topology
            // (sub-30 km² microstates: Vatican, Monaco, Nauru, Tuvalu, etc.)
            const dots = Object.entries(MICRO_NATIONS)
              .filter(([alpha3, coords]) =>
                !renderedAsPolygon.has(alpha3) &&
                isOnVisibleHemisphere(coords[0], coords[1], rotation),
              )
              .map(([alpha3, coords]) => {
                const fb   = feedbackMap[alpha3];
                ...
                return (
                  ...
                  >
                    <circle
                      r={!!fb || alpha3 === selectedAlpha3 ? 9 : 6}
                      fill={fill}
                      stroke={GLOBE_BORDER_COLOR}
                      strokeWidth={1.5}
                      style={{
                        cursor: disabled ? 'default' : 'pointer',
                        transition: 'r 0.05s, fill 0.05s',
                        opacity: dimmedCountries.has(alpha3) && !fb ? 0.35 : 1,
                      }}
                    />
                  </Marker>
